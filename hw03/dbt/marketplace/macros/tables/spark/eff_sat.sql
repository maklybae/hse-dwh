{#
    Spark override for automate_dv eff_sat macro.
...
{# Selecting the most recent records for each link hashkey -- subquery replaces QUALIFY #}
latest_records AS (
    SELECT {{ automate_dv.alias_all(source_cols, 'lr_sub') }}
    FROM (
