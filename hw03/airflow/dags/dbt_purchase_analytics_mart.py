...
with DAG(
    dag_id='dbt_purchase_analytics_mart',
    default_args=default_args,
    description='Daily full refresh of purchase analytics mart',
    start_date=datetime(2024, 1, 2),
    schedule='@daily',
    catchup=False,
    max_active_runs=1,
    tags=['dbt', 'presentation', 'purchase_analytics'],
) as dag:

    build_purchase_analytics_mart = DockerOperator(
        task_id='build_purchase_analytics_mart',
        image=DBT_IMAGE,
        ...
        command=[
            'build',
            '--profiles-dir', '.',
            '--select', '+fct_purchase_analytics_daily',
        ],
