-- Описание:
--   Когортный анализ клиентов для понимания их поведения во времени.
--   Когорта - группа клиентов, совершивших первую покупку в одном месяце.
--
-- Выходные метрики:
--   - cohort_month: месяц первой покупки когорты
--   - cohort_size: количество новых клиентов в когорте
--   - period_0_pct - period_5_pct: процент активных клиентов в каждом последующем месяце
--   - total_cohort_revenue: общая выручка от когорты за все периоды
--   - avg_revenue_per_customer: средняя выручка на клиента в когорте
--
-- Использование:
--   psql "postgresql://postgres:postgres@localhost:5433/postgres" -f <PATH_TO>/cohort_analysis.sql
